...
def url_map(lat, lon, width=300, height=300, marker_size='tiny', map_type="roadmap"):
    return u"http://maps.googleapis.com/maps/api/staticmap?center=%s,%s&zoom=15&size=%sx%s&maptype=%s&key=%s&markers=%s,%s,%sblue" % (str(lat), str(lon), str(width), str(height), map_type, settings.GOOGLE_API_KEY, str(lat), str(lon), marker_size)


def url_map_points(points, width=300, height=300, create_path=True, marker_size='tiny', map_type="terrain"):
    if len(points) == 1:
        lat = points[0][1]
        lon = points[0][0]
